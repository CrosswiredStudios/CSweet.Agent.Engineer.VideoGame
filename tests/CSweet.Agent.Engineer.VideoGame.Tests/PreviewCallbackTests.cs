...
public sealed class PreviewCallbackTests
{
    [Fact] public async Task Preview_callback_routes_typed_tests_without_entering_the_coding_harness()
    {
        ...
        var result = await runtime.ExecuteCapabilityAsync(new CSweet.Agent.Engineer.VideoGame.SpecialistAgent(), "web-preview.manage.v1",
    ...
    {
        var result = await new AgentTestRuntime().ExecuteCapabilityAsync(new CSweet.Agent.Engineer.VideoGame.SpecialistAgent(), "web-preview.manage.v1",
    ...
    [Fact] public async Task Delayed_and_duplicate_wakes_report_current_state_instead_of_old_event_state()
    {
        ...
        var id = Guid.NewGuid(); var handler = new CSweet.Agent.Engineer.VideoGame.SpecialistAgent();
    ...
    [Fact] public async Task Recovery_callback_lists_previews_without_a_previous_preview_identity()
    {
        ...
        var result = await runtime.ExecuteCapabilityAsync(new CSweet.Agent.Engineer.VideoGame.SpecialistAgent(), "web-preview.manage.v1", new { action = "list", request = new ListPreviewsRequest(project) });
