...
public sealed class AssignmentWireTests
{
    ...
    public void HostWireInputPreservesExactRoleAndPackageEvidence(bool camelCase)
    {
        ...
        var input = new WorkExecutionInputV1(Guid.NewGuid(), Guid.NewGuid(), 1,
            ...
            {
                DelegationRecommendations = [new("specialist-execution", "game-engineer", ["work.execution.run.v1"], null, true, "Engineering")],
        ...
        {
            AssignmentRequirements = new("game-engineer", [], [], ["work.execution.run.v1"]),
        ...
        var result = SpecialistAssignmentValidator.Validate(assignment, "game-engineer");
        ...
        Assert.Throws<UnauthorizedAccessException>(() => SpecialistAssignmentValidator.Validate(assignment, "game-qa"));
    }
}
